...
From both datasets all rows are unique, unique returns the same number of columns.

From the second dataset we want to incorporate the gender column.

```{r}
unique(education_data)
unique(gender_code_data)
```
...
According to the following graph, the number of HBO bachelor graduates are far greater than the other degrees over the years. After that, there are most WO Master graduates and WO Bachelor graduates. Data about HBO Master graduates are not in the graph. For WO Bachelors and WO Masters there is increase in the number of graduates over the years. For HBO Bachelors there is and increase up to around 2017, then a decrease and then again an increase of the number of graduates. However overall this the number of graduates of the HBO Bachelor has also grown. 
