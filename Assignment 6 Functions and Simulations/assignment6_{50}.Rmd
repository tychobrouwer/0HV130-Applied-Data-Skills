...
density_plot <- ggplot() + 
  ...
  geom_hline(yintercept = Q3_2, color = 'red') +
...
```{r}
sample_sizes <- rep(50, 50000)
Q4_1 = sim_values <- sapply(sample_sizes, cor_from_rnorm, correlation_size=0.4)
```
...
norm_sim_plot <- ggplot() + 
  geom_histogram(aes(Q4_1), color= 'blue', fill = 'lightblue', alpha = 0.5, bins = 30) + 
  geom_vline(xintercept = Q4_2, color = 'red') +
  labs(x = 'Simulated correlation', y = 'Observations', title = 'Histogram plot for a simulated confidence interval for a correlation of 0.4.')
